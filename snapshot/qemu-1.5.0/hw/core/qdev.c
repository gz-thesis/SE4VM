...
// DISC device snapshot. only work for PCI dev now
#include "hw/pci/pci.h"
#include "hw/pci/pci_bus.h"
#include "qom/object.h"
#include "disc/disc.h"
int device_in_DISC_list(DeviceState *dev);
...
/*
 * DISC Function:
 * Snapshot device state right after device realization (init)
 *
 * Kodos & Rohit
 */
static void device_snapshot(DeviceState *dev)
{
 DeviceClass *dc = DEVICE_GET_CLASS(dev);
 const VMStateDescription * vmsd = dc->vmsd;
 void * dev_snapshot = NULL;
 void * swapspace = NULL;
 const char * dev_name = NULL;
 unsigned int dev_state_size = 
	dc->parent_class.type->instance_size;

 if (g_num_dss >= MAX_DEV_SNAPSHOTS) {
	printf("Did not allocate space for %d devices.\n",
		g_num_dss+1);
	printf("Failed to make snapshots.\n");
	return; 
 }

 dev_snapshot = (void *)malloc(dev_state_size); 
 if (!dev_snapshot) {
	printf("Failed to alloc memory for dev snapshot\n");
	return;
 }
 /* allocate a tmp swap space for future flipping activities */
 swapspace = (void *)malloc(dev_state_size);
 if (!swapspace) {
	printf("Failed to swap space for dev snapshot\n");
	free(dev_snapshot);
	return;
 }

 memcpy(dev_snapshot, (const void *)dev, dev_state_size);
  
 if (vmsd->name){
     // dev_name = (char *)malloc(strlen(vmsd->name));
     dev_name = vmsd->name;
 }
	
 g_dss[g_num_dss].devname = dev_name;
 g_dss[g_num_dss].statesize = dev_state_size;
 g_dss[g_num_dss].snapshot = dev_snapshot;
 g_dss[g_num_dss].swapspace = swapspace;
 g_dss[g_num_dss].vmsd = vmsd;
 
 g_num_dss++;

 //printf("%s | %zx\n", dc->parent_class.type->name, 
 //			dc->parent_class.type->instance_size);
 // DeviceClass -> VMStateDescription -> .name
 // printf("device state info %s\n", vmsd->name);  

 return;
}

/* 
 * Remove the snapshot if device is removed.
 */
static void free_device_snapshot(DeviceState *dev)
{
...
/*
 * DISC Function:
 * Check if the device is specified to be checked by DISC
 *
 * Kodos & Rohit
 */
int device_in_DISC_list(DeviceState *dev)
{
	//DeviceClass *dc = DEVICE_CLASS(dev);
	PCIDevice *pci_dev = (PCIDevice *)dev;  
	// Only support PCI now
 	PCIDeviceClass *pc;
 
	if (strncmp(dev->parent_bus->name, "pci", 3) == 0) {
		pc = PCI_DEVICE_GET_CLASS(pci_dev);

		// Currently assuming e1000: vendor_id=0x8086
		//			     device_id=0x100e		
		// Debug
		...
		printf ("Device Description:%s\n", 
			pc->parent_class.desc);

		if (pc->parent_class.vmsd!=NULL)
			printf ("Name from Device VMState %s\n", 
				pc->parent_class.vmsd->name);
	
		if ( (pc->vendor_id == 0x8086) &&
                     (pc->device_id == 0x100e))
                        return 1;
...
{   
    ...
    if (dc->init) {
	   ...
	   if (device_in_DISC_list(dev)){ /* Added by DISC project */  
            device_snapshot(dev);
       }
...
static void device_unrealize(DeviceState *dev, Error **errp)
{
    ...
    if (dc->exit) {
        ...
        if (device_in_DISC_list(dev))   /* DISC project */
                free_device_snapshot(dev);     
